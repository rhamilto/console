...
test.describe('Cluster Settings when ClusterVersion Upgradeable=False', { tag: ['@admin'] }, () => {
  test('displays alerts and badges for blocked updates', async ({ page }) => {
    ...
    await test.step('Open and verify "Other available paths" modal', async () => {
      ...
      const moreUpdatesButton = firstChannel.locator('[data-test="cv-more-updates-button"]');
      ...
      const modalNotUpgradeableAlert = modal.locator(
      ...
      const blockedBadge = modal.locator('[data-test="cv-update-blocked"]');
    ...
    await test.step('Click blocked version dot and verify blocked info', async () => {
      const firstChannel = page.getByTestId('cv-channel').first();
      const blockedDot = firstChannel.locator('[data-test="cv-channel-version-dot-blocked"]');
      ...
      await expect(blockedInfo).not.toBeVisible();
    });
    ...
    await test.step('Open update modal and verify blocked updates in dropdown', async () => {
      ...
      // Open dropdown
      const dropdownToggle = modal.locator('[data-test="dropdown-with-switch-toggle"]');
      await expect(dropdownToggle).toBeVisible();
      await dropdownToggle.click();

      // Verify 2 blocked update badges in dropdown
      const blockedBadges = page.getByTestId('cv-update-blocked');
