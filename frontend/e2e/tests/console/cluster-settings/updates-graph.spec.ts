...
test.describe('Cluster Settings updates graph', { tag: ['@admin'] }, () => {
  test('displays when an update is in progress', async ({ page }) => {
    ...
    await test.step('Verify first channel (stable-4.16) details', async () => {
      ...
      const versionDots = firstChannel.locator('[data-test="cv-channel-version-dot"]');
      await expect(versionDots).toHaveCount(2);

      const versions = firstChannel.locator('[data-test="cv-channel-version"]');
