...
test.describe(`${crd} CRD`, { tag: ['@admin'] }, () => {
  ...
  test.beforeEach(async ({ k8sClient: client }) => {
    ...
    // Create test project
    await k8sClient.createProject(projectName);
  ...
  test.afterEach(async () => {
    ...
    try {
      await k8sClient.deleteProject(projectName);
    } catch (error) {
      // Ignore if already deleted
    }
  });
  ...
  test(`creates, displays, modifies, and deletes a new ${crd} instance`, async ({ page }) => {
    ...
    await test.step('Create Pod with matching label in test namespace', async () => {
      ...
      await expect(page.getByTestId('yaml-error')).not.toBeVisible();
    ...
    await test.step('Verify external log link appears on Pod logs page', async () => {
      ...
      // Give the console time to load and render the ConsoleExternalLogLink
      // The link is fetched asynchronously when the logs page mounts
      await page.waitForTimeout(2000);

      // Verify the external log link appears
