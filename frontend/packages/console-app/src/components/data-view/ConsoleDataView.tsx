...
>({
  ...
  // Calculate banner state and indeterminate checkbox state
  const bannerState = useMemo(() => {
    if (!selection || !loaded || filteredData.length === 0) {
      return { show: false, allSelected: false, isIndeterminate: false };
    }
    ...
    // Count selected items among visible items
    ...
    // All items are selected if selected count equals total count
    const allSelected = selectedCount === totalCount;

    // Indeterminate state: some (but not all) visible items are selected
  ...
  return mock ? (
    ...
    >
      ...
      >
        ...
        />
        {bannerState.show && (
          <Banner className="pf-v6-u-mb-md">
            {bannerState.allSelected ? (
              ...
              <>
                <Trans
                  ns="public"
                  i18nKey="All <1>{{count}}</1> {{label}} on this page are selected."
                >
                  All <strong>{{ count: visibleItems.length }}</strong>{' '}
                  {{ label: label || 'items' }} on this page are selected.
                </Trans>{' '}
